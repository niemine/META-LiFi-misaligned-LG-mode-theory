...
(* ::Title:: *)
(*Misaligned OAM-mode calculation for l0 =10*)
...
(*B[l_,l0_,n_,j_,k_]:=2^(1/2 (1-2 j+Abs[l0]-Abs[l-n]-Abs[n])) Sqrt[l0!] Gamma[1/2 (Abs[l0]+Abs[l-n]+Abs[l-l0-n]+2k) + 1]/(Factorial[Abs[l0]]Factorial[Abs[l-l0-n]]Factorial[Abs[n]]Factorial[Abs[l-n]]Factorial[j]Factorial[ Abs[n]+j]Factorial[k]Factorial[Abs[l-l0-n]+k])*)
...
(* ::Text:: *)
(*Next one is c = 0: First n = +-1. These contributions are twice (takes into account m = +-1, n= 0)*)
...
(* ::Text:: *)
(*We  see the final output is something logical. If phi = 0, then the last term is 0, and does not contribute to the error. If Phi = pi/2, then the last term is 1, and adds to error. If phi= -pi/2, it reduces error*)
